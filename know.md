...
1. **不用自动化工具对实名SRC扫描** — sqlmap/awvs/nessus 会被检测，用 Fiddler/Burp 手动测
2. **不把目标网站打崩** — 并发控制在 10-50 次，不对生产环境压测
3. **不涉及线上真实用户数据** — 最多用2个自己注册的账号验证
4. **不使用在线XSS平台** — 同款平台有人被抓你也完蛋
5. **没授权不碰** — 只在 SRC 授权范围内测试
6. **BC站不碰** — 博彩相关远离
7. **情报漏洞不做** — 只提交技术漏洞
8. **数据库漏洞只读2-3行** — 证明存在即可，不拖库
9. **公益SRC谨慎** — 某些顺排行榜抓人

### 测试规范

- SQL注入：只读取2-3行数据验证存在，不用 sqlmap
